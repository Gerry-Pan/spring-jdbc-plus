...
public class QueryMapper {
	...
	/**
	 * Map the {@link Sort} object to apply field name mapping using
	 * {@link RelationalPersistentEntity the type to read}.
	 *
	 * @param sort   must not be {@literal null}.
	 * @param entity related {@link RelationalPersistentEntity}, can be
	 *               {@literal null}.
	 * @return
	 */
	...
	/**
	 * Map the {@link Expression} object to apply field name mapping using
	 * {@link RelationalPersistentEntity the type to read}.
	 *
	...
	/**
	 * Map a {@link CriteriaDefinition} object into {@link Condition} and consider
	 * value/{@code NULL} bindings.
	 *
	 * @param parameterSource bind parameterSource object, must not be
	 *                        {@literal null}.
	 * @param criteria        criteria definition to map, must not be
	 *                        {@literal null}.
	...
	private Condition mapEmbeddedObjectCondition(CriteriaDefinition criteria, MapSqlParameterSource parameterSource,
		...
		RelationalPersistentEntity<?> persistentEntity = this.mappingContext
				.getRequiredPersistentEntity(embeddedProperty);

		Assert.isInstanceOf(persistentEntity.getType(), criteria.getValue(), () -> "Value must be of type "
				+ persistentEntity.getType().getName() + " for embedded entity matching");
	...
	private Condition createCondition(Column column, @Nullable Object mappedValue, int sqlType,
		...
		if (comparator == Comparator.BETWEEN || comparator == Comparator.NOT_BETWEEN) {
			...
			Expression begin = bind(pair.getFirst(), sqlType, parameterSource, column.getName().getReference(),
					ignoreCase);
			Expression end = bind(pair.getSecond(), sqlType, parameterSource, column.getName().getReference(),
					ignoreCase);
	...
	protected static class MetadataBackedField extends Field {
		...
		/**
		 * Creates a new {@link MetadataBackedField} with the given name,
		 * {@link RelationalPersistentEntity} and {@link MappingContext}.
		 *
		 * @param name      must not be {@literal null} or empty.
		...
		/**
		 * Creates a new {@link MetadataBackedField} with the given name,
		 * {@link RelationalPersistentEntity} and {@link MappingContext} with the given
		...
		/**
		 * Returns the {@link PersistentPropertyPath} for the given
		 * {@code pathExpression}.
		 *
		...
		private boolean isPathToJavaLangClassProperty(PropertyPath path) {
			return path.getType().equals(Class.class)
					&& path.getLeafProperty().getOwningType().getType().equals(Class.class);
		}
		 ...
		 * 
		 * @see
		 * org.springframework.data.mongodb.core.convert.QueryMapper.Field#isEmbedded()
		 */
		@Override
		public boolean isEmbedded() {
		 ...
		 * 
		 * @see
		 * org.springframework.data.mongodb.core.convert.QueryMapper.Field#getTypeHint()
		 */
		@Override
		public TypeInformation<?> getTypeHint() {
		 ...
		 * 
		 * @see
		 * org.springframework.data.mongodb.core.convert.QueryMapper.Field#getSqlType()
		 */
		@Override
		public int getSqlType() {
			return this.sqlType;
